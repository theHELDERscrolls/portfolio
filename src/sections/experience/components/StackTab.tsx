  ...
  return (
    <div className="flex flex-col gap-8">
      {TECH_STACK.map((category) => {
        ...
        globalIdx += category.items.length;
          ...
          <div key={category.id} className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              {category.items.map((item, i) => (
                <div
                  key={item.name}
                  className="flex flex-col items-center justify-center gap-3 p-2 w-25 h-25 border-2 border-neutral-900 bg-neutral-100 shadow-[3px_3px_0_0_#000] animate-pop"
                  style={{ animationDelay: `${(startIdx + i) * 45}ms` }}
                >
                  <img
                    src={item.url}
                    alt={item.name}
